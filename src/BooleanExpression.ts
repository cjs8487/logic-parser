...
export type ReducerArg<T> =
    | { isReduced: true; accumulator: T; item: T }
    | { isReduced: false; accumulator: T; item: string };
...
class BooleanExpression {
    ...
    reduce<T>({
        ...
        if (this.isAnd()) {
            return _.reduce(
                this.items,
                (acc, item) => andReducer(reducerArguments(acc, item)),
                andInitialValue,
            );
        }
        ...
        if (this.isOr()) {
            return _.reduce(
                this.items,
                (acc, item) => orReducer(reducerArguments(acc, item)),
                orInitialValue,
            );
        }
    ...
    getUpdatedParentItems(parentItems: ParentItems) {
        return _.mergeWith(
            ...
            (objectValue: Item[], sourceValue: Item[]) => {
                if (_.isArray(objectValue)) {
                    return _.concat(
                        objectValue,
                        _.filter(
                            sourceValue,
                            (value) => !BooleanExpression.isExpression(value),
                        ),
                    );
                }
                return undefined;
    ...
    removeDuplicateExpressions(implies: BinOp<string>): BooleanExpression {
        ...
        if (
            this.type === Op.Or &&
            newItems.length >= 2 &&
            _.every(_.map(newItems, BooleanExpression.isExpression))
        ) {
            ...
            _.forEach(booleanItems[0].items, (item) => {
                if (
                    _.every(
                        _.map(booleanItems, (expr) =>
                            _.includes(expr.items, item),
                        ),
                    )
                ) {
